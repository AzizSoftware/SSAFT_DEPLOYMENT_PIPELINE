const transactionApiBaseUrl = `http://${window.location.hostname}:7000`;
...
document.getElementById('dataForm').addEventListener('submit', async function (e) {
  ...
  try {
    if (method === 'file') {
      ...
      const res = await fetch(`${transactionApiBaseUrl}/upload-file`, {
        method: 'POST',
        body: formData
      });
      ...
      const res = await fetch(`${transactionApiBaseUrl}/use-mongo`, {
        method: 'POST',
